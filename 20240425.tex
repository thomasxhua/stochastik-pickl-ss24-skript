...
Sei $\Omega$ eine Menge, $\mathcal{A}$ $\sigma$-Algebra. Eine Abbildung $\PP\colon\mathcal{A}\to\RR$ hei\ss{}t \trt{Wahrscheinlichkeitsma\ss{}} $:\LRA$
\abc{
\item $\PP(\Omega)=1$,
\item $\PP(A)\geq0,\ \forall A\in\mathcal{A}$,
\item $\PP(\bigcup_{n=1}^\infty A_n)=\sum_{n=1}^\infty\PP(A_n)$, falls $A_n\in\mathcal{A},\ A_n\cap A_m=\emptyset\ \forall n\neq m$.
}

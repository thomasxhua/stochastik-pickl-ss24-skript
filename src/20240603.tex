...
f\"ur die Parameter $\mu\in\RR$, $\sigma\in\RR_0^+$ und $C$, sodass $\int_{-\infty}^{\infty}\rho(x)\,dx=1$ nennt man \trt{normalverteilt}.
\\~\\
...
\meq{
    ...
    \\\textabove{Polarkoord.}{=}\ &\int_{0}^{\infty}2r\pi\cdot e^{-\frac{r^2}{2}}\,dr
...
Lassen wir weg.
\subsubsection{Satz}
Der Erwartungswert ist linear, d.h. gegeben $X,Y\colon\Omega\to\RR$ Zufallsvariablen, $a\in\RR$:
...
\item diskret:
\meq{
\mathbb{E}(X+aY)&=\sum_{\mathclap{z\in(X+aY)(\Omega)}}z\PP(X+aY=z)\\
\text{(falls $|\Omega|<\infty$)}\ &=\sum_{\omega\in\Omega}(X+aY)(\omega)\cdot\PP(\{\omega\})\\
&=\sum_{\omega\in\Omega}(X(\omega)+aY(\omega))(\omega)\cdot\PP(\{\omega\})\\
...
Ein Ereignis $A$ hei\ss{}t ``\trt{fast sicher}'', falls $\PP(A)=1$.
\subsubsection{Satz}
\abc{
\item Falls $X=Y$ fast sicher, so ist $\mathbb{E}(X)=\mathbb{E}(Y)$.
\item Sei $X\equiv\mu\in\RR$, so ist $\mathbb{E}(X)=\mu$.
}
\subsubsection{Beweis}
\abc{
\item $\mathbb{E}(X-Y)$ ist fast sicher $=0\RA\mathbb{E}(X-Y)=0$,
\item klar.
}
